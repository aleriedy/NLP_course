...
def declare_placeholders(self):
    ...
    self.learning_rate_ph = tf.placeholder(dtype=tf.int32, shape=[]) 
...
def build_layers(self, vocabulary_size, embedding_dim, n_hidden_rnn, n_tags):
    ...
    embedding_matrix_variable = tf.variable(initial_embedding_martrix, dtype = tf.float32)######### YOUR CODE HERE #############
    
    # Create RNN cells (for example, tf.nn.rnn_cell.BasicLSTMCell) with n_hidden_rnn number of units 
    # and dropout (tf.nn.rnn_cell.DropoutWrapper), initializing all *_keep_prob with dropout placeholder.
    forward_cell =  tf.nn.rnn.DroupoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(n_hidden_rnn),
                                                input_keep_prob=self.dropout_ph,
                                                output_keep_prob=self.dropout_ph,
                                                state_keep_prob=self.dropout_ph)######### YOUR CODE HERE #############
    backward_cell =  tf.nn.rnn.DroupoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(n_hidden_rnn),
                                                input_keep_prob=self.dropout_ph,
                                                output_keep_prob=self.dropout_ph,
                                                state_keep_prob=self.dropout_ph)######### YOUR CODE HERE #############
    ...
    embeddings =  tf.nn.emdedding_lookup(embedding_matrix_variable, self.input_batch) ######### YOUR CODE HERE #############
    
    # Pass them through Bidirectional Dynamic RNN (tf.nn.bidirectional_dynamic_rnn).
    # Shape: [batch_size, sequence_len, 2 * n_hidden_rnn]. 
    # Also don't forget to initialize sequence_length as self.lengths and dtype as tf.float32.
    (rnn_output_fw, rnn_output_bw), _ =  tf.nn.bidirectional_dynamic_rnn(forward_cell,backward_cell, embeddings, dtype = tf.float32, sequence_length= self.lengths)######### YOUR CODE HERE #############
...
def compute_loss(self, n_tags, PAD_index):
    ...
    # Create cross entropy function function (v)
    ...
    self.loss =  tf.reduce(mask*loss_tensor)######### YOUR CODE HERE #############
